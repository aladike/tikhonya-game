...
export class World {
  ...
  trees:{x:number;z:number;s:number}[]=[]; crowns:T.InstancedMesh[]=[]; crownScales:number[]=[]; dummy=new T.Object3D();
  constructor(scene:T.Scene){
    ...
    this.house=home(this.group);this.obstacles.push({x:0,z:14,r:2.5});
    ...
    for(let i=0;i<110;i++){
      const a=random()*Math.PI*2,r=12+random()*20,x=Math.sin(a)*r,z=Math.cos(a)*r;
      if((Math.abs(x)<8&&z<17&&z>-20)||(Math.hypot(x,z-14)<12))continue;
    ...
    for(const mesh of [trunk,leaves,tops]){mesh.castShadow=true;mesh.receiveShadow=true;this.group.add(mesh);if(mesh===trunk)this.cameraObjects.push(mesh);}
    this.trees=trees;this.crowns=[leaves,tops];this.crownScales=trees.map(()=>1);
  ...
  reveal(camera:T.Vector3,hero:T.Vector3,dt:number){
    const direction=hero.clone().sub(camera);direction.y=0;direction.normalize();
    const end=hero.clone().addScaledVector(direction,7),segment=end.clone().sub(camera);segment.y=0;
    this.trees.forEach((tree,i)=>{
      const relative=new T.Vector3(tree.x-camera.x,0,tree.z-camera.z);
      const t=T.MathUtils.clamp(relative.dot(segment)/segment.lengthSq(),0,1);
      const near=relative.addScaledVector(segment,-t).length()<4.2;
      this.crownScales[i]=T.MathUtils.lerp(this.crownScales[i],near?.015:1,Math.min(1,dt*10));
      this.crowns.forEach((mesh,j)=>{this.dummy.position.set(tree.x,(j===0?3.9:5.4)*tree.s,tree.z);this.dummy.scale.setScalar(tree.s*this.crownScales[i]);this.dummy.updateMatrix();mesh.setMatrixAt(i,this.dummy.matrix);});
    });
    ...
    const between=this.house.position.clone().sub(camera),segmentToHero=hero.clone().sub(camera);
    const t=between.dot(segmentToHero)/segmentToHero.lengthSq();
    this.house.visible=!(t>0&&t<.92&&between.addScaledVector(segmentToHero,-t).length()<3.5);
  }
